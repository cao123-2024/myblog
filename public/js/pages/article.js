var _commentPollInterval = null;

function startCommentPoll(articleId) {
  if (_commentPollInterval) clearInterval(_commentPollInterval);
  _commentPollInterval = setInterval(async function() {
    try {
      var d = await API.get('/comments/article/' + articleId);
      var list = document.getElementById('comment-list');
      if (list) list.innerHTML = renderComments(d.comments || []);
    } catch(e) {}
  }, 5000);
}

function stopCommentPoll() {
  if (_commentPollInterval) { clearInterval(_commentPollInterval); _commentPollInterval = null; }
}
function render_article(articleId) { stopCommentPoll();
...
async function loadArticle(wrap, articleId) {
  try {
    ...
    wrap.innerHTML = ''
     + '<div class="stagger">'
     + (hasCover ? '<div class="article-detail-cover" style="background-image:' + cssUrl(a.cover_image) + '"></div>' : '')
     + '<h1 class="article-detail-title">'+escapeHtml(a.title)+'</h1>'
     + '<div class="article-author-card card-glass">'
      + '<div class="article-author-avatar" style="background-image:' + cssUrl(authorAv) + '" onclick="navigate(\'profile\','+(a.author?.id||'')+')"></div>'
     + '<div class="article-author-info">'
      ...
      + (otherImages.length > 0 ? '<div class="article-extra-images">'+otherImages.map(function(img){ return '<img src="'+escapeHtml(img)+'" loading="lazy" />'; }).join('')+'</div>' : '')
      + '</div>'
      + '<div class="mt-8">'
...
function renderComments(comments) {
  ...
  return comments.map(c => ''
   + '<div class="comment-item">'
    + '<div class="comment-avatar" style="background-image:' + cssUrl(avatarUrl(c.author)) + '" onclick="navigate(\'profile\','+c.author?.id+')"></div>'
